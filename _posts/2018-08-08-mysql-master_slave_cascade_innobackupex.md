---
layout: post
title: 使用innobackupex在线搭建级联复制
date: 2018-08-08
categories: blog
tags: [MySQL]
description: 使用innobackupex在线搭建级联复制
---
...
```sql
[mysql@node1 ~]$ innobackupex  --apply-log /home/mysql/db_full_backup
180808 02:42:52 innobackupex: Starting the apply-log operation
...
ib_logfile1                                                                                                                          100%   48MB  48.0MB/s   00:01
ibtmp1                                                                                                                               100%   12MB  12.0MB/s   00:00
...
[mysql@node1 ~]$ scp -r /home/mysql/db_full_backup 192.168.2.63:/home/mysql/
...
setup_actors.frm                                                                                                                     100% 8701     8.5KB/s   00:00
ib_buffer_pool                                                                                                                       100%  357     0.4KB/s   00:00
ib_logfile1                                                                                                                          100%   48MB  48.0MB/s   00:00
ibtmp1                                                                                                                               100%   12MB  12.0MB/s   00:01
backup-my.cnf                                                                                                                        100%  424     0.4KB/s   00:00
db.MYI                                                                                                                               100% 5120     5.0KB/s   00:00
plugin.ibd                                                                                                                           100%   96KB  96.0KB/s   00:00
db.MYD                                                                                                                               100%  976     1.0KB/s   00:00
...
[mysql@node3 ~]$ cp -r /home/mysql/db_full_backup/ /mysql1/data

```

### 查看一级从库配置文件
- 注意: 此文件中参数log-slave-updates必须开启 
-       此参数的作用为将应用 relaylog产生的数据变化记录到binlog中

```sql

[mysql@node2 ~]$ cat  /mysql1/mysql1.
mysql1.cnf  mysql1.log  mysql1.pid
...
[mysql@node3 ~]$ mysqld --defaults-file=/mysql1/mysql1.cnf &
[1] 3010

```


### 开启二级从库(node3)到一级从库(node2)的复制

```sql

mysql> change master to master_host='192.168.2.62',master_port=3306,master_user='dao',master_password='dao',master_log_file='mysql-bin.000001',master_log_pos=154 ;    Query OK, 0 rows affected, 2 warnings (0.02 sec)
...
[mysql@node2 db_full_backup]$ cat xtrabackup_info
...
mysql>  change master to master_host='192.168.2.61',master_port=3306,master_user='dao',master_password='dao',master_log_file='mysql-bin.000003',master_log_pos=596  ;  Query OK, 0 rows affected, 2 warnings (0.02 sec)
...
mysql> create database test_ms_cascade ;
Query OK, 1 row affected (0.01 sec)


mysql> create table test_ms_cascade.t1 (c1 int) ;
Query OK, 0 rows affected (0.03 sec)

mysql> insert into test_ms_cascade.t1 values(1) ;
